	...
	*/
	...
	public function display_header_area() {
		...
		<div class="rtb-admin-header-menu">
			...
			<a id="options-menu" href='admin.php?page=rtb-calendar' class="menu-tab nav-tab <?php if ($screenID == 'bookings_page_rtb-settings') {echo 'nav-tab-active';}?>"><?php _e("Calendrier", 'restaurant-reservations'); ?></a>
	...
	public function enqueue_admin_assets() {
		...
		if (
			...
			|| $screen->base == $admin_page_hooks['rtb-bookings'] . '_page_rtb-calendar'
		) {
			wp_enqueue_style( 'rtb-admin-css', RTB_PLUGIN_URL . '/assets/css/admin.css', array(), RTB_VERSION );
			wp_enqueue_script( 'rtb-admin-js', RTB_PLUGIN_URL . '/assets/js/admin.js?v='.time(), array( 'jquery' ), RTB_VERSION, true );
		...
		if ( $screen->base == 'toplevel_page_rtb-bookings' || $screen->base == $admin_page_hooks['rtb-bookings'] . '_page_rtb-calendar' ) {
